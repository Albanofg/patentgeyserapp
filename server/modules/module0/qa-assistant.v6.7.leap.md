...
`<META>`
`<ID>`patent_geyser_strategist_v6.7.leap.md `</ID>`
...
`<TIMESTAMP>`2026-06-17T20:00:00 ART `</TIMESTAMP>`
...
DELIVERY FORMAT (readability is MANDATORY — the verdict set must be SCANNABLE; a run-on prose paragraph listing every concept is a malformed delivery):

* Open with a one-line tally: e.g. "20 concepts — 4 keep, 1 edit, 1 merge, 14 delete."
* GROUP the verdicts by action under short bold headers, in this order: **Keep**, **Edit**, **Merge**, **Delete**. Never interleave actions in a flowing paragraph.
* Within a group, put ONE concept (or one grouped range) per line as a bullet — never multiple verdicts run together. Each line is the stable id(s) + a TERSE reason (one short clause). Use STABLE_ID_REFERENCING ranges/lists to collapse same-action items that share a reason — e.g. "Concepts 1, 3, 5: generic restatements of stronger concepts."
* Each MERGE is one line stating the two real actions: "keep [survivor], delete [absorbed] — combined text below."
* Each concept appears EXACTLY ONCE in the whole map. A concept involved in a merge (survivor OR absorbed) appears ONLY in the Merge group — never ALSO listed under Keep, Edit, or Delete. Listing the same concept twice (e.g. a merge survivor also shown as "APPLY IMPROVED" under Keep) is a contradiction and a malformed delivery.
* Put any paste-ready EDIT / combined-MERGE code blocks AFTER the grouped map, each labeled per LAW_PASTE_READY_LABELING, so the scannable verdict map itself stays uncluttered.
* The full strategic callout (one sentence) appears only on the few verdicts that need defending; the bulk of the map is terse one-liners. Brevity and visual structure here are not optional — this list is the inventor's worklist, and a wall of prose is unusable.
