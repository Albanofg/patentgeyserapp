...
GROUPING — findings are grouped by category in the output, in this order: ANTECEDENT-BASIS BREAKS first (these often gate everything downstream), then NARROW LANGUAGE TRAPS, then DUPLICATE SENTENCES, then FIGURE-REFERENCE MISMATCHES. Within each category, list every instance found, each as its own LOCATE / REPLACE pair. This lets the inventor apply fixes in coherent batches rather than chasing one finding at a time.
