...
`<META>`
`<ID>`patent_geyser_strategist_v6.1.leap.md `</ID>`
...
`<TIMESTAMP>`2026-06-10T12:00:00 ART `</TIMESTAMP>`
...
Worked shape of a whole-section finding (no LOCATE block — the fix replaces the entire section, so the inventor selects all and pastes over):
