...
// Native EPD/NMI checkout — same flow as /buy, plus a private coupon code
// field. Card data is tokenized in-browser by Collect.js (loaded from
// secure.nmi.com) so we never see PAN/CVV on our servers. We forward full
// billing info to NMI for AVS + CVV checks — anti-fraud wall.
//
// Coupon codes are validated (and re-validated at charge time) server-side —
// the discounted price shown here is a preview only, never trusted for the
// actual charge.
...
function packPriceNumber(price: string): number {
...
export default function Purchase() {
  ...
  // Coupon — validated (preview only) via /api/public/validate-coupon. The
  // actual discount is re-checked server-side at charge time; nothing here
  // is trusted for the real amount.
  ...
  const submissionRef = useRef<{ buyer: Buyer; packId: PackId; couponCode: string | null } | null>(null);
  ...
  async function applyCoupon(codeOverride?: string) {
    ...
    try {
      ...
      if (data?.valid) {
        setAppliedCoupon({ code, percentOff: data.percentOff });
        setCouponInput(code);
      } else {
  ...
  // Pre-fill and auto-validate a coupon shared via a direct link (?code=...).
  useEffect(() => {
  ...
  async function finishCheckout(
    ctx: { buyer: Buyer; packId: PackId; couponCode: string | null },
    paymentToken: string,
  ) {
    try {
      const res = await fetch("/api/checkout/epd", {
        ...
        body: JSON.stringify({
          ...
          packId: ctx.packId,
          paymentToken,
          ...(ctx.couponCode ? { couponCode: ctx.couponCode } : {}),
        }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
      ...
      // server issued. The /purchase page swaps to the password form inline;
      // no email, no separate page.
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    console.log("[purchase] submit fired", { collectReady, hasCollect: !!window.CollectJS });
    setErrorMsg(null);
    if (!collectReady || !window.CollectJS) {
      console.log("[purchase] bailed: collect not ready");
      setErrorMsg("Card form not ready yet. Wait a moment and try again.");
      return;
    }
    const v = validate();
    if (v) {
      console.log("[purchase] bailed: validation", v);
      setErrorMsg(v);
      return;
    }
    ...
    submissionRef.current = { buyer: trimmed, packId, couponCode: appliedCoupon?.code || null };
    setSubmitting(true);
    ...
    console.log("[purchase] calling startPaymentRequest");
    window.CollectJS.startPaymentRequest();
  }

  const selectedPack = PACKS.find((p) => p.id === packId)!;
  const selectedPackNumeric = packPriceNumber(selectedPack.price);
  const discountedTotal = appliedCoupon
    ? selectedPackNumeric * (100 - appliedCoupon.percentOff) / 100
    : selectedPackNumeric;
          ...
          <form onSubmit={handleSubmit} className="p-6 pt-3 space-y-6">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {PACKS.map((p) => {
                ...
                const numeric = packPriceNumber(p.price);
                const discounted = appliedCoupon
                  ? numeric * (100 - appliedCoupon.percentOff) / 100
                  : null;
                return (
                  <button
                    type="button"
            ...
            <Button type="submit" className="w-full" disabled={submitting || !collectReady || !!configError}>
              {submitting ? "Processing…" : `Pay ${formatUsd(discountedTotal)}`}
            </Button>
